...
%
    ...
    assert(mmfile~=-1,['File' filename ' not found']);
...
%       Matrix Market site.    
    ...
    assert(strcmp(head0,'%%MatrixMarket'),'Not a valid MatrixMarket header.');
...
%% Read size information, then branch according to sparse or dense format
    ...
    if(strcmp(rep,'coordinate')) %  read matrix given in sparse coordinate matrix format
        ...
        while(~count)
            ...
            [sizeinfo,count] = sscanf(commentline,'%d%d%d');
            assert(count==0 || count==3,'Invalid size specification line.')
        end
        rows = sizeinfo(1);
        cols = sizeinfo(2);
        entries = sizeinfo(3);
    ...
    elseif (strcmp(rep,'array')) %  read matrix given in dense 
        ...
        while(count==0)
            ...
            [sizeinfo,count] = sscanf(commentline,'%d%d');
            assert(count==0 || count==2,'Invalid specification line.');
        end
        rows = sizeinfo(1);
        cols = sizeinfo(2);
        entries = rows*cols;
        ...
        elseif(strcmp(field,'complex'))         % complx valued entries:
            ...
            A  = [tmpr+tmpi*1i; zeros(entries-1,1)];
            for j=2:entries
                tmpr = fscanf(mmfile,'%f',1);
                tmpi = fscanf(mmfile,'%f',1);
                A(j)  = tmpr + tmpi*1i;
...
%% If symmetric, skew-symmetric or Hermitian, duplicate lower triangular part and modify entries as appropriate:
